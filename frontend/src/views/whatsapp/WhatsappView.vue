...
const editingInstancia = ref<WhatsappInstancia | null>(null)
...
function openEditAgente(instancia: WhatsappInstancia) {
  editingInstancia.value = instancia
  editAgenteId.value = instancia.agente_id
}
...
async function salvarAgente() {
  ...
  try {
    const res = await api.updateInstancia(editingInstancia.value.id, { agente_id: editAgenteId.value })
...
<template>
  <div class="h-full overflow-y-auto p-8 bg-slate-50 dark:bg-slate-900">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">WhatsApp</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Gerencie instâncias de atendimento via WhatsApp</p>
      </div>
      <button
        @click="openCreate"
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-slate-100 dark:border-slate-700 text-left">
            <th class="px-5 py-3 text-slate-500 dark:text-slate-400 font-medium">Instância</th>
        ...
        <tbody>
          ...
          >
            <td class="px-5 py-3 font-mono text-slate-700 dark:text-slate-200">{{ inst.instance_name }}</td>
            ...
            <td class="px-5 py-3">
              <div class="flex items-center gap-2">
                <button
                  @click="conectar(inst)"
                  class="px-3 py-1 rounded text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
                >
                  Conectar
                </button>
                <button
    ...
    <div v-else class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-slate-600 dark:text-slate-300 font-medium">Nenhuma instância cadastrada</p>
      <p class="text-slate-400 dark:text-slate-500 text-sm mt-1">Crie uma instância para começar a atender no WhatsApp</p>
      <button
        @click="openCreate"
    ...
    >
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-sm p-6">
        ...
        <p class="text-slate-500 dark:text-slate-400 text-sm mb-4">{{ editingInstancia.instance_name }}</p>
        <select
    ...
    >
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md p-6">
        <h2 class="font-semibold text-slate-800 dark:text-slate-100 mb-5">Nova instância WhatsApp</h2>

        <div class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Nome da instância</label>
            <input
            ...
            />
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Sem espaços. Será o identificador no Evolution API.</p>
          </div>
    ...
    <div
      ...
      style="background: rgba(0,0,0,0.5)"
    >
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
        <h2 class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Conectar WhatsApp</h2>
        <p class="text-slate-500 dark:text-slate-400 text-sm mb-5">Escaneie o QR code com o celular</p>

        <div
          v-if="!qrBase64"
          class="w-56 h-56 mx-auto flex items-center justify-center rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-600"
        >
          <div class="text-slate-400 dark:text-slate-500 text-sm">Aguardando QR code...</div>
        </div>
        ...
        <img
          ...
          class="w-56 h-56 mx-auto rounded-xl border border-slate-200 dark:border-slate-600 object-contain"
        />
        ...
        <button
          @click="fecharQrModal"
          class="mt-5 px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white transition-colors"
        >
          Fechar
        </button>
      </div>
