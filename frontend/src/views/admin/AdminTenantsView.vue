...
async function submitCreate() {
  ...
  createError.value = ''
  try {
    const tenantRes = await adminApi.createTenant({
      nome: createForm.value.nome,
      descricao: createForm.value.descricao || undefined,
    })
...
<template>
  <div class="p-8">
    ...
    <!-- Modal: criar tenant + owner -->
    <div
      v-if="showCreateForm"
      class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
      @click.self="showCreateForm = false"
    >
      <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]">
        <h2 class="text-slate-800 font-semibold mb-4">Novo Tenant</h2>

        <div class="space-y-3">
          <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide">Tenant</p>
          <div>
            <label class="block text-slate-600 text-sm mb-1">Nome <span class="text-red-400">*</span></label>
            <input
          ...
          <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide pt-2">Usuário owner</p>
          <div>
            <label class="block text-slate-600 text-sm mb-1">Username <span class="text-red-400">*</span></label>
            <input
          ...
          <div>
            <label class="block text-slate-600 text-sm mb-1">Nome completo</label>
            <input
              v-model="createForm.owner_nome"
              placeholder="Ex: João Silva"
              class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-400"
            />
          </div>
          <div>
            <label class="block text-slate-600 text-sm mb-1">E-mail</label>
            <input
          ...
          <div>
            <label class="block text-slate-600 text-sm mb-1">Senha <span class="text-red-400">*</span></label>
            <input
    ...
    >
      <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h2 class="text-slate-800 font-semibold mb-4">Editar Tenant</h2>
        <div class="space-y-3">
          <div>
          ...
          <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide pt-2">Configuração LLM</p>
          <div>
            <label class="block text-slate-600 text-sm mb-1">Provider</label>
            <select
              v-model="editForm.llm_provider"
              class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-400"
            >
          ...
          <div v-if="editForm.llm_provider && editForm.llm_provider !== 'ollama'">
            <label class="block text-slate-600 text-sm mb-1">Modelo</label>
            <input
              v-model="editForm.llm_model"
              placeholder="Ex: gpt-4o-mini, llama-3.1-8b-instant, claude-3-haiku-20240307"
              class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-400"
            />
          </div>
          <div v-if="editForm.llm_provider && editForm.llm_provider !== 'ollama'">
            <label class="block text-slate-600 text-sm mb-1">
              API Key
              <span v-if="editingTenant?.llm_api_key_set" class="ml-1 text-xs text-green-600">(configurada)</span>
              <span v-else class="ml-1 text-xs text-slate-400">(não configurada)</span>
            </label>
            <input
              v-model="editForm.llm_api_key"
              type="password"
    ...
    >
      <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] flex flex-col">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-slate-800 font-semibold">
            Usuários — {{ usuariosTenant.nome }}
          </h2>
          <button @click="usuariosTenant = null" class="text-slate-400 hover:text-slate-600 text-xl leading-none">&times;</button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto space-y-2 mb-4">
          ...
          >
            <div>
              ...
              <span class="ml-2 text-slate-400">{{ u.nome }}</span>
              <span
                class="ml-2 text-xs px-1.5 py-0.5 rounded font-medium"
                :class="u.role === 'OWNER' ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-500'"
              >
                {{ u.role }}
              </span>
            </div>
        ...
        <!-- Add user -->
        <div v-if="showAddUser" class="border-t border-slate-200 pt-4 space-y-3">
          <p class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Novo usuário</p>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-slate-600 text-xs mb-1">Username *</label>
              <input v-model="addUserForm.username" class="w-full border border-slate-200 rounded px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label class="block text-slate-600 text-xs mb-1">Senha *</label>
              <input v-model="addUserForm.password" type="password" class="w-full border border-slate-200 rounded px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label class="block text-slate-600 text-xs mb-1">Nome</label>
              <input v-model="addUserForm.nome" class="w-full border border-slate-200 rounded px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label class="block text-slate-600 text-xs mb-1">E-mail</label>
              <input v-model="addUserForm.email" type="email" class="w-full border border-slate-200 rounded px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label class="block text-slate-600 text-xs mb-1">Role</label>
              <select v-model="addUserForm.role" class="w-full border border-slate-200 rounded px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-violet-400">
          ...
          <div class="flex gap-2">
            ...
            <button @click="showAddUser = false" class="text-slate-500 hover:text-slate-700 text-sm px-3">Cancelar</button>
          </div>
        </div>
        ...
        <button
          ...
          class="mt-3 w-full border border-dashed border-violet-300 text-violet-600 hover:bg-violet-50 rounded-lg py-2 text-sm transition-colors"
        >
          + Adicionar usuário
        </button>
      </div>
